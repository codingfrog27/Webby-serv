...
void	Server::connectionAction(Connection &connect, pollfd &poll, size_t i)
{
	...
	{
		if (connect._isServerSocket)  //could be part of enum
			acceptNewConnects(i);
		else if (connect._CStatus == connectStatus::IDLE ||\
				 connect._CStatus == connectStatus::READING)
			connect._CStatus = connect._request.readRequest();
	}
	else if ((poll.revents & POLLOUT) && \
			(connect._CStatus == connectStatus::DONE_READING \
